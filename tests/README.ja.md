...
- `transparent/` — 透過転送（SPEC §22）：オーバーレイが全分割数・両軸・ダブルバッファ有無でピクセル一致（split=1 = 面全体を 1 回マスク付き push＝参照実装）・角丸ダイアログの下で下地が実際に残る・全面を覆うシーンでは影響なし・透過色の C++ 型でマスク対象が変わらない・モード／色の切り替えはタイルのバイト列が同一でも差分ハッシュを無効化
