...
方針：**まず実験して判断する**。内部 sprite の per-pixel clip だけで上記ケースが全面描画と一致するなら、`y -= offsetY` 転送のみで提供する。clip だけでは破綻する（描画位置やソース切り出しがずれる）場合で、専用クリッピングの実装が重いと判断したら、当面**非対応メソッド**としてもよい（§8。呼ぶとコンパイルエラー）。
