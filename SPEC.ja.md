...
| T1-16 | 透過転送 | オーバーレイが全分割数・両軸で一致すること（split=1 = 面全体を 1 回マスク付き push）。マスクで下のレイヤが実際に残ること（角丸）。全面を覆うシーンでは影響が無いこと。色の C++ 型でマスク対象が変わらないこと。モード／色の切り替えがタイルのバイト列が同一でも差分ハッシュを無効化すること（§22.9） |
...
## 22. 透過転送（オーバーレイ／ダイアログ）

指定した 1 色だけを転送から除外し、すでにパネルに出ている絵を透けさせるオプション機能。**既定では無効**で、差分転送（§21）と違ってメモリを一切消費しない。`render()` の代わりに `renderTransparent()` を呼んだフレームだけで有効になる。

### 22.1 目標設定と割り切り

出発点はダイアログである。既存の画面はそのまま残し、ダイアログだけを上に描く。面を透過色で塗り、その上に描かれた部分だけを転送すれば、全画面フレームバッファなしにこれが成立する。

- **矩形のオーバーレイにこの機能は不要。** ダイアログ位置に置いた `LGFXVirtualSprite` なら、その矩形だけを再描画・転送できる（§7.1）し、そのほうが速い。透過転送は**矩形でない**オーバーレイ（角丸、影、散在するウィジェット、枠のない文字）のためのものである。
- **α 合成ではない。** カラーキーによるマスクであり、画素は転送されるかされないかのどちらかである。半透明はパネルの読み戻し（多くのパネルで不可能）か全画面バッファを要する。`pushAlphaImage()` による合成は別機能として将来の拡張に置く（§22.8）。
- **転送画素あたりのコストは `render()` より高い**（§22.6）。描画コールバックも全タイルで走る。得をするのは「送る画素数が大幅に減る」からであって、転送自体が安くなるわけではない。
- **レイヤ管理はしない。** どのレイヤをいつ・どの順で描くかはアプリの責任。ライブラリは 1 色を転送しないだけである。
...
template <typename T> void setTransparentColor(const T& color);  // 既定は上記
uint32_t transparentColor(void) const;                           // RGB888 で返る

// LGFXVirtualScreen / LGFXVirtualSprite: render() の各形に対応する renderTransparent()
...
// LGFXVirtualSprite では位置指定形（…, int x, int y）も同様に用意する
```

**色は状態にし、そのフレームが透過かどうかは状態にしない。** これは意図的な分割である。

- 色は**コンテンツの性質**（描画が絶対に作らない値はどれか）である。ほぼ変わらないし、呼び出しごとにマジック定数を書くのはノイズになる。妥当な既定値があるので、普通のケースではセッターに触れずに済む。
- 透過は**そのフレームの性質**（レイヤなのか、絵全体なのか）である。オブジェクトに持たせないことで、`render()` が残留モードを引き継ぐ事故が起きない。引き継いだ場合の壊れ方は静かで悪質だ（背景を失うか、背景色が透過色と一致した箇所に穴が空く）。

**`render()` の引数（モード enum や色引数）ではなくメソッド名を分ける理由：**

1. `LGFXVirtualSprite::render()` はすでに位置 `x, y` と `ctx` を取る。末尾にモード引数を足すと `render(draw, state, 40, 80, Transparent)` になり、呼び出し行から数値の意味が読めない。モードを名前に移せばこの問題自体が消える。
2. 発見しやすい。`render` と打てば補完に `renderTransparent` が並ぶ。§6.3 が専用具象クラスの利点として挙げているのと同じ理由である。
3. enum にしたくなる残りの動機（将来のモード追加）は実際には来ない。合成方式が本質的に違うもの（α）はどうせ別名が適切で、チューニングつまみ（全透過タイルのスキップ、bbox クリップ — §22.8）はフレームごとの判断ではないので `setDiffMode()` と同じくセッターに属する。

内部では protected な `Transfer { Opaque, Transparent }` enum としてレンダ経路に渡す。将来モードを足しても公開シグネチャは変わらない。

### 22.3 タイルの初期化

`renderTransparent()` は**タイルを `transparentColor()` で自動クリアする**（`setBackgroundColor()` は使わない）。したがって描画コールバック側は何も特別なことをしなくてよく、描かなかった場所がそのままパネルを透かす。

代替案（コールバック自身に透過色で塗らせる）は却下した。マスク付き push に渡す値としてライブラリが色を知る必要があるためどちらにせよセッターは必要で、かつ塗り忘れは「下のレイヤを消す不透明な全面転送」に静かに化ける。

`setAutoClear(false)` と `renderTransparent()` の併用は前タイルの残骸がバッファに残るためサポート対象外とする（§11.1 の「初期状態は未定義」と同じ扱い。ただし壊れ方はより悪い＝残骸は不透明なので転送される）。

### 22.4 透過色

既定は RGB565 `0x0120`、LovyanGFX が `TFT_TRANSPARENT` として公開している値である。どこに渡しても RGB565 として解釈されるよう、意図的に `int` 型にしてある（理由は以下）。

**色の C++ 型が、LovyanGFX の解釈を決める。** `color_conv_t::convert()` は引数の型でディスパッチしており、`uint32_t` は RGB888、`int` / `int32_t` / `uint16_t`（`TFT_*` 定数や `color565()` の戻り値）は RGB565 として読まれる。そこで `setTransparentColor()` は型を保つテンプレートとし、保存時に `convert_to_rgb888()` で RGB888 に正規化する。RGB565 → RGB888 → 各深度の往復は上位ビットの複製展開なので無損失である。これをしないと `setTransparentColor(TFT_TRANSPARENT)` は RGB888 の `0x000120` を保存してしまい、スケッチが描く `TFT_TRANSPARENT` とは**別の色**になる。壊れ方は最悪で、何もマスクされず、オーバーレイが下のレイヤを消す。

衝突してはいけないのは**タイルの色深度に変換した後の値**である。コールバックが描いた画素は、透過色と同じ値に変換された時点で転送から外れる。帰結：

- 8 bpp（`rgb332`）では 16 bpp よりずっと多くの色が同じ値に潰れる。16 bpp で安全な色が 8 bpp では安全でないことがある。
- パレット深度（4/8 bpp + パレット）では値は**パレット index**。1 つ予約すること。
- 空き色がまったく無い場合、逃げ道は別の色ではなく別の機構である。オーバーレイの外接矩形に `LGFXVirtualSprite` を置く（§22.1）。

### 22.5 差分転送との関係と無効化

両機能は共存できる。タイルのハッシュはタイルの**内容**を表しており、内容が変わっていないタイルのスキップは不透明転送のときと同様に正しい。

変わるのは前提である。差分転送は「転送しない = パネルにはハッシュが表す絵がまだ出ている」を仮定する（§21.5）。透過オーバーレイではパネルは**2 レイヤの合成**を保持しているので、契約が 1 つ増える：**オーバーレイの下にあるものを何かが描き直したら、オーバーレイ側で `invalidate()` を呼ぶ。** これは §21.5 の「自分以外が画面に触ったら呼ぶ」と同じ規則だが、下のレイヤを別マネージャが描くのが意図された構成であるぶん、通常運用で発火する。

§21.5 の検出対象に加えて、以下の 2 つは自動検出する。

- `render()` と `renderTransparent()` の切り替え
- `setTransparentColor()` の変更

どちらも「直前のレンダで使った透過色、不透明なら範囲外マーカー（bit 24。`pixelcopy_t::NON_TRANSP` と同じ手法）」という 1 つの値に畳んで監視する。これはハッシュでは検出できないケースで効く：自動クリア色が透過色と等しいとき、`render()` と `renderTransparent()` は**バイト単位で同一のタイル**を作るのに、パネルの結果は異なる。

### 22.6 マスク付き転送の実際

タイルは `LGFX_Sprite::pushSprite(dst, x, y, transp)` で push され、`param->transp` を設定した状態で `Panel_LCD::writeImage()` に到達する（LovyanGFX 1.2.21 / 1.2.26 で確認）。この経路は不透明経路とは構造が違う。

- **1 走査線ずつ**処理し、`fp_skip` で透過画素を飛ばし、残った run ごとに DMA バッファへコピー → ウィンドウ設定 → 書き込みを行う。
- **全透過の走査線はバス転送ゼロ**（スキップ走査だけ）。これがほぼ空のオーバーレイを安くしている。
- したがってコストは「タイルあたり 1 回のウィンドウ設定」ではなく「**run あたり 1 回のウィンドウ設定** + 全タイルの全画素走査」になる。行あたり短い run が多数（点線模様）が最悪ケース、少数のベタ矩形が最良ケース。
- 各 run はスプライトからコピーされてから書き込まれるので、DMA が飛んでいる間にタイルバッファを読まれることはない。よってこのモードでダブルバッファに利得はない。無害ではあるため、あらゆるパネル実装への仮定を置いて特別扱いするのではなく、既存の `waitDMA()` 方針（§10.5）をそのまま残す。

`diffPushedPixels()` は push したタイルの画素数を数えるので、`renderTransparent()` では実際にパネルへ届いた量の**上限値**になる。真の値を出すには push 中に run を数える必要があり、LovyanGFX はそれを公開していない。

### 22.7 不変条件

1. **`render()` は一切変わらない。** `renderTransparent` と書かない呼び出しは、ビット単位で従来どおりに振る舞う。これにより既存の §13 テスト群がそのままこの機能の回帰テストになる。
2. **`split=1` が参照実装。** 1 タイルなら「面全体を 1 回マスク付き push する」であり、手書きの LovyanGFX コードとまったく同じである。他のすべての分割数・両分割軸はこれと pixel 一致しなければならない（§13.2 の方法をマスク付き転送に適用したもの）。
3. **ライブラリが特別扱いするのは透過色 1 色だけ。** 2 色目もα も画素ごとのコールバックも持たない。
4. **§21.7-4（「出力画素は一切変わらない」）は差分転送の不変条件であり、ライブラリ全体の不変条件ではない。** 透過転送は設計上出力画素を変える。だからこそ差分転送のモードではなく、独自の参照（不変条件 2）を持つ別機能として置いている。

### 22.8 将来の拡張点

いずれも §21.8 の「描画後シーム」に入る純粋追加である。

- **全透過タイルの push を完全にスキップ。** 全面が透過色のままのタイルは push 不要。差分を有効にしていればほぼ無料で判定できる（全透過タイルのハッシュと比較するだけ）。単独でも線形走査 1 回。
- **描画された bbox に絞って push。** 非透過範囲を 1 パスで求め、push 前に `setClipRect()` する（クリップで実転送が絞られることは §21.6 で確認済み）。走査範囲とウィンドウ設定回数の両方が減る。
- **α 合成**（`pushAlphaImage()` 経由、`renderAlpha()`）。タイルにα チャネル（`argb8888`）が必要になる別機構であり、この機能のモードではない。
- **`LGFXVirtualCanvas::fillTransparent()`** — 描き終えたオーバーレイから穴を開け直すには canvas 側が透過色を知る必要があり、現状は知らない。

### 22.9 テスト方針

§13 の枠組みに乗る（`tests/transparent/`）。

- **出力不変性（最重要）**：オーバーレイを split 1 / 3 / 7 / 13、両軸、ダブルバッファ有無で描き、すべて pixel 一致すること（不変条件 2）。マスク値の変換経路が変わる 24 bpp も含む。
- **実際にマスクされていること**：角丸ダイアログの角の画素は下のレイヤが残り、本体部分は残らない。
- **マスクする対象が無いときに影響が無いこと**：全面を覆うシーンは、透過 push と不透明 push で一致する。
- **色の型**：同じ色を RGB565 の `TFT_*` 定数として渡した場合と RGB888 の `uint32_t` として渡した場合で、マスクされる画素が同じであること（§22.4）。
- **差分との合成**：変化のないオーバーレイは何も転送しない。モード切り替えと色変更はそれぞれ転送を強制する（自動クリア色＝透過色でタイルのバイト列が同一になるケースを含む — §22.5）。
- **`LGFXVirtualSprite`**：配置済みタイルスプライトの透過 push が単一タイルと複数タイルで一致し、自分の形の外を塗らないこと。
