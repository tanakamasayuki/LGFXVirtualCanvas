...
既定では **3タイル**。普通は気にするのはタイル数より RAM なので、予算を指定すると
収まる最大のタイル高をライブラリが選びます：
...
| メンバ | 説明 |
|---|---|
| `LGFXVirtualScreen(LovyanGFX& panel, int splitCount = 0)` | パネル上に構築。`0` = auto（既定3タイル）。この時点では未確保。 |
...
| `void setDoubleBuffer(bool enable)` | タイルバッファを2枚使い、あるタイルの DMA 転送と次タイルの描画を重ねる（高速、タイルRAM 2倍。既定 `false`）。SPEC §10.5 参照。 |
