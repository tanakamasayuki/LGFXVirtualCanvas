...
- (EN) **Fix:** `setBackgroundColor()` misread the color. It took a plain `uint32_t`, which LovyanGFX reads as RGB888, so passing an RGB565 constant filled the wrong color — `setBackgroundColor(TFT_NAVY)` produced RGB `(0,0,8)` instead of navy `(0,0,123)`. It is now a template that preserves the value's C++ type (`TFT_*` / `int` / `uint16_t` = RGB565, `uint32_t` = RGB888), like every other color argument. **Sketches that passed a `TFT_*` constant or `color565()` result will see their auto-clear color change to the color they actually asked for**; `uint32_t` / `color888()` callers are unaffected. `tests/autoclear` now pins the fill against the same constant drawn straight to the panel.
- (JA) **修正:** `setBackgroundColor()` が色を誤解釈していた。素の `uint32_t` を取っていたため LovyanGFX には RGB888 として読まれ、RGB565 の定数を渡すと別の色で塗られていた（`setBackgroundColor(TFT_NAVY)` は navy `(0,0,123)` ではなく `(0,0,8)`）。他の色引数と同様、値の C++ 型を保つテンプレートに変更した（`TFT_*` / `int` / `uint16_t` は RGB565、`uint32_t` は RGB888）。**`TFT_*` 定数や `color565()` を渡していたスケッチでは auto-clear の色が「本来指定したはずの色」に変わる**。`uint32_t` / `color888()` を渡していた場合は影響なし。`tests/autoclear` に、同じ定数を直接パネルに描いた色との一致を固定するテストを追加。
